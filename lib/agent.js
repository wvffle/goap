...
class Agent {
  ...
  plan () {
    ...
    start.calc_heuristic(this)

    opened.add(start)

    while (!opened.isEmpty()) {
      // console.log(opened.array.map(n => n.action && n.action.name))
      ...
      // check all goals
      let inState = true
      for (const [name, fn] of this.goals) {
        if (!node.parent || !fn(node.parent.state, node.state, this.name)) {
          inState = false
          break
        }
      }
      //console.log('================')
      //console.log(inState, node.action && node.action.name)
