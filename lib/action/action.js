...
class Action extends Events {
  ...
  perform(world) {
    ...
    const meets = false;
    for (let p in this.preconditions) {
      const precon = this.preconditions[p];
      if (precon.func !== undefined) {
        meets = precon.func(precon.value);
      } else {
        meets = world.state[p] === precon.value;
      }
